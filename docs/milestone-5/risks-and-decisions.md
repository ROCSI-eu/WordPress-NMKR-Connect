...
| `M5-R02` | Naming | `NMKR Connect` / `nmkr-connect` may require trademark/project-name authorization or a safer slug/name strategy | Critical | MITIGATED BY OWNER DECISION — issue #111 selects the non-brand-leading `Connector for NMKR` identity with `connector-for-nmkr` as the candidate slug/text domain; directory assignment remains external | Release identity and canonical main-file alignment are merged; do not claim the slug is approved/reserved until WordPress.org assigns it |
...
| `M5-R13` | WordPress.org | Official Plugin Check may reject the first submission candidate because current code contains unresolved scanner/reviewer findings | Critical | MITIGATED — #132 and focused child issues resolved the code-level errors; after WordPress 7.1.1 compatibility validation and `Tested up to: 7.1`, the exact candidate passes Plugin Check 2.1.0 with 0 errors and 310 audited residual warnings | Preserve the audited warning classifications, rerun official tooling on the final exact submission ZIP, and reopen remediation only for a newly identified blocker |
...
| `M5-D14` | Use `1.0.0` as the first public production/WordPress.org release version | Treats Milestone 5 as the first stable public release rather than an early prototype | SUPERSEDED BY M5-D16 — issue #107 records the earlier decision; issue #111 records the replacement |
| `M5-D15` | Use `Connector for NMKR` as the WordPress.org public display name and `connector-for-nmkr` as the candidate directory slug, gettext text domain, and release-package directory; preserve `nmkr-connect.php` and established `nmkr_*` runtime contracts | Uses a clear non-brand-leading third-party integration identity without forcing a compatibility-breaking rename of established technical contracts | PARTIALLY SUPERSEDED BY M5-D17 — display/slug/text-domain and `nmkr_*` contract decisions remain accepted; only the filename-retention clause is superseded |
| `M5-D16` | Use `0.25.0` as the first public stable release and follow the repository pre-1.0 versioning policy | Preserves room to mature public/persistent contracts before the `1.0.0` compatibility commitment while still publishing numeric stable WordPress.org releases | ACCEPTED — owner decision recorded in issue #111; see `docs/versioning.md` |
| `M5-D17` | Before the first WordPress.org submission, use `connector-for-nmkr.php` as the canonical public main plugin file; preserve the `connector-for-nmkr` directory/text-domain identity and established `nmkr_*` runtime/persistence contracts; migrate private pre-public installations operationally | Removes the avoidable directory/main-file mismatch without broadening the change into internal contract renames or a public compatibility shim | ACCEPTED — issue #126; supersedes only M5-D15's filename-retention clause |
