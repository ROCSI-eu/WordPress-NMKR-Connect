...
| `M5-R13` | WordPress.org | Official Plugin Check may reject the first submission candidate because current code contains unresolved scanner/reviewer findings | Critical | OPEN — #127 Plugin Check 2.1.0 baseline found 86 errors and 569 warnings; #132 classifies the findings and #133–#136 track the 85 code-level errors, while `Tested up to` remains a later exact-candidate gate | Resolve the focused code-level blockers without broad warning-chasing, rebuild one exact candidate, rerun Plugin Check, then resume #127 compatibility/readme/submission gates |
