...
`nmkr_connect_schema_version` versions the database schema separately from the plugin version. Normal plugin loading runs a site-scoped, token-owned upgrade lock. Schema v3 verifies the nullable `char(36)` unique `run_id` contract and the nullable integer `items_skipped` and `token_details_synced` columns before recording success. Existing pre-v3 rows retain `NULL` for those new counters (unknown); every new run explicitly writes zero or a positive value.

Synchronization keeps the lifecycle vocabulary `completed`, `failed`, and `stopped`. A pure `terminal_result` projection adds result quality without creating another lifecycle: failed and stopped project directly, while completed runs become `success`, `completed_with_skips`, or `completed_with_errors` based on authoritative history counters, with failures taking precedence over skips. The five counters are `items_processed`, `items_successful`, `items_failed`, `items_skipped`, and `token_details_synced`. Verified progress and dashboard responses expose only the lifecycle, derived result, and these safe counters; raw history and private failure diagnostics are not public/admin response fields.
