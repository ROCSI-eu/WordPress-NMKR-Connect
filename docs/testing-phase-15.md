...
HTTP readiness checks a same-origin static WordPress core asset rather than `wp-login.php`, the site root, or another PHP/bootstrap URL. Normal TLS certificate verification remains enabled. A private CA bundle can be supplied through `NMKR_PHASE2_CURL_CA_BUNDLE` in the private environment file when needed. Redirects are rejected and never followed; any 3xx response fails without printing the raw origin, redirect target, or effective URL.
