...
Follow the [testing-environment access guide](testing-environment-access.md). Its public preparation defines one public-safe Google Drive folder containing four documents aligned to Reports of Testing Results, Security Implementation Evidence, Documentation Evidence, and Testing Environment Access. The folder may be linked from the public Catalyst Proof of Achievement, but it and the form exclude temporary login details and every other private value. The private preflight passed, but folder submission and access delivery remain pending. An assessor requests temporary access through the support route and identifies one of the three generic capability profiles; a suitable existing dedicated account may be reused after credential, capability, login, and intended-page revalidation. Supply temporary login details separately, never publish or forward them, record the authorized window privately, and rotate or revoke access when it ends. After access is supplied, ordinary one-at-a-time dashboard Start, progress, completion, Stop, and abort observation are permitted; concurrent, repetitive, load, stress, fault-injection, destructive, direct-state, and infrastructure-level activity are prohibited.

Credentials, endpoints, account identities, temporary login details, populated folder and support values, Drive links, support addresses, authentication state, logs, private evidence, and infrastructure details are excluded from Git history, the public folder and form, and public discussion. M3-21 remains **Implemented, validation pending** until actual public-folder submission, separate temporary-access delivery when requested, and EVD-013 registration are completed.
