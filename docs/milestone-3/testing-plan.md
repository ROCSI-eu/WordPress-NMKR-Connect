...
Follow the [testing-environment access guide](testing-environment-access.md). One public-safe Google Drive folder containing four documents aligned to Reports of Testing Results, Security Implementation Evidence, Documentation Evidence, and Testing Environment Access was submitted through the public Catalyst Proof of Achievement at `2026-08-28 11:53 UTC`. Its Testing Environment Access document and the form may contain the project-selected support route; this repository describes that route only generically and contains no populated value. The folder and form exclude temporary login details, account email addresses, authentication state, private evidence locations, and other credentials. The private integrity gates passed and all three generic profiles were revalidated and ready. An assessor may request temporary access through the operational support route during the active documented window and identify a profile. Supply temporary login details separately when requested, never publish or forward them, and retain the handoff privately. At submission, access was available on request and no request had been received. After access is supplied, ordinary one-at-a-time dashboard Start, progress, completion, Stop, and abort observation are permitted; concurrent, repetitive, load, stress, fault-injection, destructive, direct-state, and infrastructure-level activity are prohibited.

Credentials, endpoints, account identities, temporary login details, authentication state, logs, private evidence locations, and infrastructure details are excluded from Git history and public discussion and, except for the project-selected public support route, from the public folder and form. [EVD-013](executed-evidence-2026-08-28.md) registers the completed public-folder delivery and operational on-request access state, so M3-21 is **Validated** within that disclosed scope. The support route must remain operational through the active window, with expiry or reassessment at `2026-10-01 00:00 UTC`.
