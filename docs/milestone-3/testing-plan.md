...
[EVD-008](executed-evidence-2026-08-21.md#evd-008--bounded-repeated-sequential-mixed-chain-traffic) is validated executed evidence for nine successful sequential cycles over the same stable 10-project, 100-unique-token dataset, with 1,089 aggregate NMKR API requests and 900 repeated token-processing operations. It is supporting/partial evidence for M3-05 and M3-11, but the guarded light profile, sequential execution, and cooldowns do not establish materially higher processing pressure, heavier API traffic, or stress; both requirements remain **Implemented, validation pending**.

The next required action is a bounded plugin-level higher-pressure test using a defined larger synthetic mixed-chain API workload, without loading the live NMKR API. It should evaluate plugin-controlled pagination, processing, database writes, deduplication, memory, metrics, terminalization, and cleanup under explicit ceilings, monitoring, stop thresholds, recovery, and final-state checks. It must not imply production, upstream, unlimited-scale, or external infrastructure certification. Streaming traversal, run-scoped deduplication, bounded provisional progress, and canonical final metrics remain implementation foundations rather than measurements by themselves; uptime and materially broader traffic claims still require their own evidence.
...
Register Cardano and Solana evidence with attributable chain representation, dataset scope, terminal state, integrity checks, API timing, cleanup, and limitations. One controlled mixed-chain execution may support both chain-specific functional claims when Cardano and Solana are each directly represented and attributable; artificial chain-only executions are unnecessary absent an express contractual requirement. EVD-001 supplies bounded light-profile support, while EVD-008 supplies supporting/partial repeated sequential mixed-chain evidence. Neither record proves heavier API traffic, concurrent stress, production capacity, maximum load, arbitrary scale, or universal behavior. Streaming traversal, run-scoped deduplication, unique final totals, provisional progress, pagination failure handling, and final metrics remain implementation foundations; the larger synthetic workload described above is still required.
