...
[EVD-008](executed-evidence-2026-08-21.md#evd-008--bounded-repeated-sequential-mixed-chain-traffic) completes the defined M3-05/M3-11 evidence action within a bounded higher-traffic and heavier mixed-chain profile: nine successful sequential cycles over the same stable 10-project, 100-unique-token dataset, with 1,089 NMKR API requests and 900 repeated token-processing operations. This validates only that disclosed repeated-traffic boundary—not concurrency, production capacity, maximum load, arbitrary scale, or all possible load/stress conditions.

For future reruns, continue to use an authorized non-production environment and a specifically defined profile with rate/concurrency ceilings, monitoring, stop thresholds, cooldown, recovery, and final-state checks. Never extrapolate unlimited scale or production capacity from a bounded profile or turn an execution into a broad production-scale stress campaign. Streaming traversal, run-scoped deduplication, bounded provisional progress, and canonical final metrics remain implementation foundations rather than measurements by themselves; uptime and materially broader traffic claims still require their own evidence.
...
Register Cardano and Solana evidence with attributable chain representation, dataset scope, terminal state, integrity checks, API timing, cleanup, and limitations. One controlled mixed-chain execution may support both chain-specific functional claims when Cardano and Solana are each directly represented and attributable; artificial chain-only executions are unnecessary absent an express contractual requirement. EVD-001 supplies bounded light-profile support, while EVD-008 supplies the completed heavier mixed-chain execution within its nine-cycle sequential repeated-traffic boundary. Neither record proves concurrent stress, production capacity, maximum load, arbitrary scale, or universal behavior. Streaming traversal, run-scoped deduplication, unique final totals, provisional progress, pagination failure handling, and final metrics remain implementation foundations; materially broader traffic claims require separate measured evidence.
