...
A controlled synchronization with a disclosed dataset, request count, outcomes, timing, resource signal, and terminal/final state may serve as proportionate bounded operational-load evidence. For a new stepped, concurrent, stress, or broader-capacity claim, use an authorized non-production environment and define rate/concurrency ceilings, monitoring, stop thresholds, cooldown, and recovery checks. Never extrapolate unlimited scale or production capacity from a bounded profile. Streaming traversal, run-scoped deduplication, bounded provisional progress, and canonical final metrics remain implementation foundations rather than measurements by themselves; uptime and materially broader traffic claims still require their own evidence.
...
Register Cardano and Solana evidence with attributable chain representation, dataset scope, terminal state, integrity checks, API timing, cleanup, and limitations. One controlled mixed-chain execution may support both when Cardano and Solana are each directly represented and attributable; artificial chain-only executions are unnecessary absent an express contractual requirement. A bounded mixed-chain run does not prove stress, high concurrency, uptime, arbitrary scale, or production capacity. Streaming traversal, run-scoped deduplication, unique final totals, provisional progress, pagination failure handling, and final metrics remain implementation foundations; materially broader traffic claims require separate measured evidence.
