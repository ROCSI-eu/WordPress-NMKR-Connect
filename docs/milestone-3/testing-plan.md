...
Follow the [testing-environment access guide](testing-environment-access.md). Its public preparation defines one public-safe Google Drive folder containing four documents aligned to Reports of Testing Results, Security Implementation Evidence, Documentation Evidence, and Testing Environment Access. The folder may be linked from the public Catalyst Proof of Achievement, and its Testing Environment Access document and the form may contain the project-selected support route; this repository describes that route only generically and contains no populated value. The folder and form exclude temporary login details, account email addresses, authentication state, private evidence locations, and other credentials. The private preflight passed and all three generic profiles are revalidated and ready, but folder submission remains pending. An assessor may request temporary access through the operational support route during the documented window and identify a profile. Supply temporary login details separately when requested, never publish or forward them, and retain the handoff privately; if no request occurs, record that fact explicitly as of evidence registration. After access is supplied, ordinary one-at-a-time dashboard Start, progress, completion, Stop, and abort observation are permitted; concurrent, repetitive, load, stress, fault-injection, destructive, direct-state, and infrastructure-level activity are prohibited.

Credentials, endpoints, account identities, temporary login details, authentication state, logs, private evidence locations, and infrastructure details are excluded from Git history and public discussion and, except for the project-selected public support route, from the public folder and form. M3-21 remains **Implemented, validation pending** until actual public-folder submission, operational support-route/window evidence, retained private evidence, a handoff record when requested or explicit no-request record, and EVD-013 registration are completed.
