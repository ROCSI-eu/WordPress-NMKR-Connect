...
The [testing-environment access guide](testing-environment-access.md) implements the public procedure for controlled non-production access. One public-safe Google Drive folder may be linked from the public Catalyst Proof of Achievement and contains four evidence documents aligned to Reports of Testing Results, Security Implementation Evidence, Documentation Evidence, and Testing Environment Access. The public Testing Environment Access document and form may contain the project-selected support route, while this repository describes it only generically and contains no populated value. Temporary login details, account email addresses, authentication state, private evidence locations, and other credentials remain excluded from that folder and form and are supplied separately through the support route after the assessor identifies one of three generic capability profiles. Existing dedicated accounts for all three profiles may be reused after credential, capability, login, and intended-page revalidation. Once access is supplied, ordinary one-at-a-time dashboard Start, progress, completion, Stop, and abort observation remain permitted; concurrent, repetitive, load, stress, fault-injection, destructive, direct-state, and infrastructure-level activity remain prohibited.

The private staging preflight revalidated three existing dedicated accounts and all three generic capability profiles, together with the required environment-integrity and health conditions, without publishing private values. The public evidence folder has not been submitted through the Catalyst form, so no access request, temporary-access handoff, expiry/revocation result, or EVD-013 is asserted. Actual folder submission, operational support-route availability during the documented window, private handoff if requested (or an explicit no-request record), and later sanitized evidence registration remain open. M3-21 is **Implemented, validation pending**, and Milestone 3 remains incomplete.
...
| M3-21 | Implemented, validation pending | The public four-document folder procedure, three revalidated generic profiles, and private preflight are ready; actual folder submission, operational support-route/window evidence, a private handoff if requested or explicit no-request record, and sanitized EVD-013 registration remain outstanding. |

## Open actions

- **M3-21:** submit the public-safe four-document folder through the Catalyst Proof of Achievement, keep the public support route operational during the documented window, supply temporary login details separately when requested or explicitly record that no request was received, retain private evidence, then create and register sanitized EVD-013.
