# ROCSI Connector for NMKR user guide
...
Obtain the ZIP from a trusted release source and confirm that it is intended for the version you plan to deploy. In **Plugins → Add New → Upload Plugin**, select the ZIP, choose **Install Now**, and then activate **ROCSI Connector for NMKR**. Use the purpose-built package produced by `npm run build:package`; it is self-contained and does not require a `vendor/` directory.
...
The exact top-level menu is **ROCSI Connector for NMKR**. Its plugin submenus are **Dashboard**, **NFT Projects**, **Shortcodes**, and **Analytics** (unless the analytics UI is disabled by the build). Configuration is separately located at **Settings → ROCSI Connector for NMKR**; the Plugins screen also provides a **Settings** action link.
...
| --- | --- |
| `nmkr_access_plugin` | Exposes the ROCSI Connector for NMKR menu shell and restricted-user routing. It does not authorize every child page or action by itself. |
...
| `nmkr_view_projects` | Views **ROCSI Connector for NMKR → NFT Projects**. |
| `nmkr_view_shortcodes` | Views **ROCSI Connector for NMKR → Shortcodes**. |
| `nmkr_view_analytics` | Views the Analytics administration page and its reporting requests. |
| `nmkr_manage_settings` | Opens and saves **Settings → ROCSI Connector for NMKR**. |
...
Only a user with `nmkr_manage_settings` can open or save **Settings → ROCSI Connector for NMKR**. WordPress settings nonces and the capability check protect saving; a rejected save should not be worked around by granting broader permissions than needed.
...
1. Save a valid API key and open **ROCSI Connector for NMKR → Dashboard**.
