...
- **Class; status:** completed documentation-only synthesis/provenance; sealed through the PR #102 merge as the final M4-10 provenance record.
...
- **Reviewed/source SHA and tree; deployed SHA:** PR #102 final head `cab99822e845e0164170ea8dd833fb403eb5a095`, tree `381ed1d0fbba87c699121d2f1d35539c4e105f7e`; deployed SHA Not applicable because M4-10 was documentation-only. Exact-head Phase 4 CI run `33514710372` (run 394) completed successfully.
- **Merged commit/tree and merged-main CI:** merged main `9d5f3710f4c1f790ee47b607171f41e66d9f2422`, tree `381ed1d0fbba87c699121d2f1d35539c4e105f7e`, is tree-equivalent to the PR #102 final head. Merged-main Phase 4 CI run `33515519482` (run 395) completed successfully; Public-safe checks, Synthetic security rendering, and Dependency audits all passed. Tree equivalence establishes documentation-content equivalence only, not runtime deployment or private validation.
...
M4-10, including its documentation-only provenance seal, is complete within its disclosed reporting scope, and Milestone 4 is delivered within the project's disclosed bounded scopes. The seal records PR #102 facts that now exist; it is part of M4-10, not M4-11, and does not recursively attempt to record its own future PR head, merge, tree, or CI. The package uses the ledger rows and linked historical records as bounded inputs, preserves `S-06` and `A-01` as open assurance gaps, keeps historical dependency results time-dependent, and keeps the M4-08 runtime identity separate from its later evidence-document merge. This is not Catalyst approval, certification, penetration testing, production security, or universal assurance.
