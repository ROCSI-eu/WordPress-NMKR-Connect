...
NMKR_PLUGIN_SLUG=rocsi-connector-for-nmkr.php
...
if "${target_base[@]}" TARGET_ACTIVE_PLUGIN_FILE="$target_fixture/rocsi-connector-for-nmkr.php" bash "$target_fixture/scripts/nmkr-phase2-test-runner.sh" >"$tmp_dir/target-binding-mismatch.output" 2>&1; then exit 1; fi
...
for profile in general existing-readonly; do
  ...
  if "${target_base[@]/NMKR_PHASE2_PROFILE=targeted-readonly/NMKR_PHASE2_PROFILE=$profile_value}" NMKR_PHASE2_TARGET_SUITE="$suite_value" NMKR_PHASE2_RUNTIME_INTEGRITY=true TARGET_ACTIVE_PLUGIN_FILE="$target_fixture/rocsi-connector-for-nmkr.php" bash "$target_fixture/scripts/nmkr-phase2-test-runner.sh" >"$tmp_dir/runtime-binding-mismatch-$profile.output" 2>&1; then exit 1; fi
