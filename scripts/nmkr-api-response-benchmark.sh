...
DEPLOYED="$(python3 - "$NMKR_DEPLOYED_PLUGIN_PATH" "$WP_PATH" "${NMKR_PLUGIN_SLUG:-connector-for-nmkr/connector-for-nmkr.php}" 2>/dev/null <<'PY'
import os,sys
...
"$WP_BIN" --path="$WP_PATH" plugin is-active "${NMKR_PLUGIN_SLUG:-connector-for-nmkr/connector-for-nmkr.php}" >/dev/null 2>&1 || fail plugin-binding
