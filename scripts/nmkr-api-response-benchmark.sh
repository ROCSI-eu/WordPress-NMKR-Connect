...
DEPLOYED="$(python3 - "$NMKR_DEPLOYED_PLUGIN_PATH" "$WP_PATH" "${NMKR_PLUGIN_SLUG:-rocsi-connector-for-nmkr/rocsi-connector-for-nmkr.php}" 2>/dev/null <<'PY'
import os,sys
...
"$WP_BIN" --path="$WP_PATH" plugin is-active "${NMKR_PLUGIN_SLUG:-rocsi-connector-for-nmkr/rocsi-connector-for-nmkr.php}" >/dev/null 2>&1 || fail plugin-binding
...
set +e
NMKR_API_BENCHMARK_CONTROLLER=1 "$TIMEOUT_BIN" --foreground --signal=TERM --kill-after=10s 310s "$WP_BIN" --path="$WP_PATH" eval-file "$DEPLOYED/scripts/nmkr-api-response-benchmark.php" >"$RESULT" 2>"$DIAG" &
CHILD_PID=$!
