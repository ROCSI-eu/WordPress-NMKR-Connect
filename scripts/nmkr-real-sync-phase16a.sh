...
validate_target_integrity() {
  ...
  wp_cli plugin is-active "${NMKR_PLUGIN_SLUG:-rocsi-connector-for-nmkr/rocsi-connector-for-nmkr.php}" >/dev/null 2>>"$DIAGNOSTIC_FILE" || fail plugin-active
  validate_admin_capability
  validate_target_git >/dev/null 2>>"$DIAGNOSTIC_FILE" || fail deployment-integrity
}

resolve_deployed_real() {
  python3 - "$REPO_ROOT" "${WP_PATH:-}" "${NMKR_PLUGIN_SLUG:-rocsi-connector-for-nmkr/rocsi-connector-for-nmkr.php}" "${NMKR_DEPLOYED_PLUGIN_PATH:-}" <<'PY'
import os, sys
...
LOCK=$(printf '%q' "$LOCK")
NMKR_PLUGIN_SLUG=$(printf '%q' "${NMKR_PLUGIN_SLUG:-rocsi-connector-for-nmkr/rocsi-connector-for-nmkr.php}")
