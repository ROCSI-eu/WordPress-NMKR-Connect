...
run wordpress "$WP_CLI_BIN" --path="$WP_ROOT" plugin is-active "${NMKR_PLUGIN_SLUG:-connector-for-nmkr/connector-for-nmkr.php}"
NMKR_DEPLOYED_PLUGIN_PATH="$DEPLOYED" NMKR_PLUGIN_SLUG="${NMKR_PLUGIN_SLUG:-connector-for-nmkr/connector-for-nmkr.php}" run deployed-source "$WP_CLI_BIN" --path="$WP_ROOT" eval '
$active=realpath(WP_PLUGIN_DIR."/".getenv("NMKR_PLUGIN_SLUG"));$expected=realpath(getenv("NMKR_DEPLOYED_PLUGIN_PATH")."/connector-for-nmkr.php");exit($active!==false&&$expected!==false&&hash_equals($expected,$active)?0:1);'
