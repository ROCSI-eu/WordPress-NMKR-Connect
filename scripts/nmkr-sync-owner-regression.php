...
$GLOBALS['options']=array();$GLOBALS['transients']=array('nmkr_sync_in_progress'=>true,'nmkr_sync_progress'=>3);nmkr_admit_sync_owner($a);update_option('nmkr_sync_in_progress',true);$wpdb->fail_lock=true;$queued_cleanup=nmkr_cleanup_failed_queued_sync($a);$wpdb->fail_lock=false;check(is_wp_error($queued_cleanup)&&nmkr_sync_owner_matches($a,'queued',0)&&!get_option('nmkr_sync_in_progress')&&!get_transient('nmkr_sync_in_progress')&&!get_transient('nmkr_sync_progress')&&get_option('nmkr_sync_status')==='failed_cleanup_pending','queued release lock failure retains owner and clears provisional markers');
