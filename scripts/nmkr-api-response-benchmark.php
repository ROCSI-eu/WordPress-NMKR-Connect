...
$project_id=function($project){return (string)($project['uid']??$project['project_uid']??$project['projectuid']??$project['projectUid']??'');};
$projects=$invoke('projects',false); if(is_wp_error($projects)||empty($projects)){$stop();} usort($projects,function($a,$b)use($project_id){return strcmp($project_id($a),$project_id($b));}); $project_uid=$project_id($projects[0]); if($project_uid===''){$stop();}
$token_id=function($token){return (string)($token['uid']??$token['token_uid']??$token['tokenUid']??'');};
$tokens=$invoke('token_list',false); if(is_wp_error($tokens)||empty($tokens)){$stop();} usort($tokens,function($a,$b)use($token_id){return strcmp($token_id($a),$token_id($b));}); $token_uid=$token_id($tokens[0]); if($token_uid===''||is_wp_error($invoke('token_detail',false))){$stop();}
