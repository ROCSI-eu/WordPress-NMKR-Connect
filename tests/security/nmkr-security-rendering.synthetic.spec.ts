...
test('production synchronization and analytics sinks render hostile-shaped values as text', async ({ page }) => {
  ...
  let reportPageError: () => void = () => {};
  const pageErrorObserved = new Promise<void>(resolve => { reportPageError = resolve; });
  page.on('pageerror', error => { pageErrors.push(error); reportPageError(); });
  ...
  await page.evaluate((marker) => {
    ...
    class JQ {
      ...
      attr(name:string, value?:unknown){ const elements=this.nodes.filter((n):n is Element=>n instanceof Element); if(arguments.length===1){const result=elements[0]?.getAttribute(name);return result===null?undefined:result;} elements.forEach(n=>n.setAttribute(name,String(value))); return this; }
      text(v?:unknown){ const nodes=this.nodes.filter((n):n is Node=>n instanceof Node); if(arguments.length){nodes.forEach(n=>n.textContent=String(v));return this;} return nodes[0]?.textContent||''; }
      empty(){this.nodes.forEach(n=>{if(n instanceof Element)n.replaceChildren();});return this;} append(child:any){this.nodes.forEach(n=>{if(n instanceof Element)n.append(...child.nodes);});return this;}
      find(sel:string){return new JQ(this.nodes.flatMap(n=>n instanceof Element?Array.from(n.querySelectorAll(sel)):[]));} each(fn:Function){this.nodes.forEach((n,i)=>fn.call(n,i,n));return this;}
      addClass(c:string){this.nodes.forEach(n=>{if(n instanceof Element)n.classList.add(c);});return this;} removeClass(c:string){this.nodes.forEach(n=>{if(n instanceof Element)n.classList.remove(c);});return this;}
    }
    const jq:any = (sel:any, attrs?:any) => {
      ...
      if(sel instanceof EventTarget) return new JQ([sel]); return new JQ(Array.from(document.querySelectorAll(sel)));
    };
  ...
  await Promise.race([
    page.waitForFunction(() => document.querySelectorAll('#nmkr-top-projects-root tbody tr').length > 0),
    pageErrorObserved,
  ]);
