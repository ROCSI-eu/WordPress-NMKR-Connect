...
function nmkr_stream_token_pages($project_uids, $fetch_page, $process_token, $checkpoint, $progress = null, $max_pages = NMKR_SYNC_MAX_TOKEN_PAGES_PER_PROJECT, $authoritative_stop = null) {
    if (!is_array($project_uids) || !is_callable($fetch_page) || !is_callable($process_token) || !is_callable($checkpoint)) {
        return new WP_Error('nmkr_token_page_invalid_configuration', __('Token pagination could not be configured.', 'rocsi-connector-for-nmkr'));
    }
    $max_pages = (int) $max_pages;
    if ($max_pages < 1) return new WP_Error('nmkr_token_page_limit', __('The token pagination safety limit is invalid.', 'rocsi-connector-for-nmkr'));
    $owners = array();
    $unique_count = 0;
    ...
    foreach (array_values($project_uids) as $project_index => $project_uid) {
        ...
        $page_number = 1;
        while (true) {
            if ($page_number > $max_pages) return new WP_Error('nmkr_token_page_limit', __('Token pagination reached the plugin safety limit before an empty terminal page.', 'rocsi-connector-for-nmkr'));
            ...
            if (!is_array($page) || (!empty($page) && array_keys($page) !== range(0, count($page) - 1))) {
                return new WP_Error('nmkr_token_page_malformed_shape', __('A token page had an invalid top-level shape.', 'rocsi-connector-for-nmkr'));
            }
            ...
            foreach ($page as $token) {
                if (!is_array($token)) return new WP_Error('nmkr_token_page_malformed_record', __('A token page contained an invalid token record.', 'rocsi-connector-for-nmkr'));
                $raw_uid = array_key_exists('uid', $token) ? $token['uid'] : (array_key_exists('token_uid', $token) ? $token['token_uid'] : null);
                if (!is_string($raw_uid)) return new WP_Error('nmkr_token_page_malformed_record', __('A token page contained an invalid token identifier.', 'rocsi-connector-for-nmkr'));
                $uid = trim($raw_uid);
                if ($uid !== $raw_uid || $uid === '' || strlen($uid) > 200 || preg_match('/^[A-Za-z0-9_-]+$/', $uid) !== 1) return new WP_Error('nmkr_token_page_malformed_record', __('A token page contained an invalid token identifier.', 'rocsi-connector-for-nmkr'));
                $uids[] = $uid;
            }
            $signature_uids = array_values(array_unique($uids)); sort($signature_uids, SORT_STRING);
            $signature = hash('sha256', implode("\n", $signature_uids));
            if (isset($signatures[$signature])) return new WP_Error('nmkr_token_page_repeated', __('Token pagination returned a repeated non-empty page.', 'rocsi-connector-for-nmkr'));
            $signatures[$signature] = true;
            $new_on_page = 0;
            foreach ($page as $offset => $token) {
                $uid = $uids[$offset];
                if (isset($owners[$uid])) {
                    if (!hash_equals((string) $owners[$uid], (string) $project_uid)) return new WP_Error('nmkr_token_project_conflict', __('A token identifier was associated with multiple projects.', 'rocsi-connector-for-nmkr'));
                    continue;
                }
            ...
            if ($new_on_page === 0) return new WP_Error('nmkr_token_page_no_progress', __('A non-empty token page contained no new token identifiers.', 'rocsi-connector-for-nmkr'));
            $halt = call_user_func($checkpoint, 'after_page_processing', $project_uid, $page_number); if (is_wp_error($halt)) return $halt;
