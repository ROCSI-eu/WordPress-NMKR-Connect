...
function nmkr_save_sync_finalization_resume($sync_stats_id, $final) {
    ...
    if ($record['outcome'] === 'failed' && !empty($final['error_message'])) {
        $error_message = nmkr_bound_sync_finalization_error_message($final['error_message']);
        if ($error_message !== '') {
...
function nmkr_build_final_sync_metrics($provided, $sync_data, $totals = array()) {
    ...
    if (!empty($sync_data['sync_stats_id']) && function_exists('nmkr_get_sync_metrics_receipt')
        && is_array(nmkr_get_sync_metrics_receipt((int) $sync_data['sync_stats_id']))
        && count(array_intersect_key($provided, array_flip($metric_keys))) === count($metric_keys)) {
        return array_intersect_key($provided, array_flip($metric_keys));
    }
    $live = get_transient('nmkr_current_sync_stats_live');
    $evidence = is_array($live) && !empty($live['request_count']) && isset($live['request_times']) ? $live : ($sync_data['api_metric_evidence'] ?? null);
    $required_evidence = array('request_count', 'successful_requests', 'failed_requests', 'total_api_time', 'request_times');
    ...
    foreach ($required_evidence as $key) {
        if (!array_key_exists($key, $evidence)) return new WP_Error('nmkr_final_metrics_incomplete', 'Completed synchronization metrics are incomplete.');
    }
    ...
    $total_api_time = 0.0;
    foreach ($evidence['request_times'] as $duration) {
        ...
        $total_api_time += (float) $duration;
    }
    if (!is_numeric($evidence['total_api_time']) || !is_finite((float) $evidence['total_api_time'])
        || (float) $evidence['total_api_time'] < 0 || abs((float) $evidence['total_api_time'] - $total_api_time) > 0.000001) {
        return new WP_Error('nmkr_final_metrics_inconsistent_duration', 'Completed synchronization timing aggregates are inconsistent.');
    }
    $sources = array(
        'total_projects' => array($totals, $provided, $sync_data),
        'total_tokens' => array($totals, $provided, $sync_data),
        'total_sync_duration' => array($provided, $sync_data),
        'memory_usage' => array($provided, $sync_data),
    );
    $metrics = array();
    foreach ($sources as $key => $candidates) {
        $found = false;
        foreach ($candidates as $candidate) if (is_array($candidate) && array_key_exists($key, $candidate) && $candidate[$key] !== null) { $metrics[$key] = $candidate[$key]; $found = true; break; }
        if (!$found || !is_numeric($metrics[$key]) || !is_finite((float) $metrics[$key]) || (float) $metrics[$key] < 0) return new WP_Error('nmkr_final_metrics_missing_field', 'Completed synchronization aggregate metrics are incomplete.');
    }
    $metrics['total_projects'] = (int) $metrics['total_projects'];
    $metrics['total_tokens'] = (int) $metrics['total_tokens'];
    $metrics['total_sync_duration'] = (float) $metrics['total_sync_duration'];
    $metrics['memory_usage'] = (float) $metrics['memory_usage'];
    $metrics['total_api_time'] = $total_api_time;
    $metrics['average_response_time'] = $total_api_time / $request_count;
    $metrics['api_requests'] = $request_count;
    return $metrics;
}
...
function nmkr_sync_finalization_resume_event_scheduled($sync_stats_id) {
    return (bool) wp_next_scheduled('nmkr_resume_sync_finalization', array((int) $sync_stats_id));
}
...
function nmkr_is_valid_sync_finalization_record($record, $run_id = '', $sync_stats_id = 0) {
    ...
    if (!is_array($record) || !nmkr_is_valid_sync_run_id((string) ($record['run_id'] ?? ''))
        ...
        || (isset($record['error_message']) && (($record['outcome'] ?? '') !== 'failed'
            || !is_string($record['error_message']) || $record['error_message'] === ''
            || strlen($record['error_message']) > 500))) {
        return false;
    }
...
function nmkr_set_sync_finalization_error() {
...
function nmkr_preserve_and_schedule_sync_finalization_retry($record, $increment_attempt = false) {
    if (!nmkr_is_valid_sync_finalization_record($record)) {
        nmkr_set_sync_finalization_error();
        return false;
    }
    if ($increment_attempt) $record['attempt']++;
    $key = nmkr_sync_finalization_resume_key($record['sync_stats_id']);
    update_option($key, $record, false);
    if (get_option($key, false) !== $record || $record['attempt'] >= 5) {
        nmkr_set_sync_finalization_error();
        return false;
    }
    if (!nmkr_schedule_sync_finalization_resume($record['sync_stats_id'], $record['attempt'])
        || !nmkr_sync_finalization_resume_event_scheduled($record['sync_stats_id'])) {
        nmkr_set_sync_finalization_error();
        return false;
    }
    return get_option($key, false) === $record;
}
...
function nmkr_prepare_sync_finalization($sync_stats_id, $final, $sync_data) {
    ...
    if ($final['outcome'] === 'completed') {
        ...
        if (is_wp_error($final['metrics'])) {
            $final['error_message'] = nmkr_bound_sync_finalization_error_message($final['metrics']->get_error_message());
    ...
    $scheduled = !empty($saved['run_id'])
        ...
        : (is_array($saved) && nmkr_schedule_sync_finalization_resume($sync_stats_id, (int) ($saved['attempt'] ?? 0))
            && nmkr_sync_finalization_resume_event_scheduled($sync_stats_id));
    if (!is_array($saved) || !$scheduled) {
        nmkr_set_sync_finalization_error();
        return false;
    }
    return $saved;
...
function nmkr_sync_finalization_handoff_pending($run_id, $sync_stats_id) {
    $record = get_option(nmkr_sync_finalization_resume_key($sync_stats_id), false);
    return nmkr_is_valid_sync_finalization_record($record, $run_id, $sync_stats_id)
        && nmkr_sync_finalization_resume_event_scheduled($sync_stats_id)
        && (nmkr_sync_owner_matches($run_id, 'running', $sync_stats_id)
...
function nmkr_resume_sync_finalization($sync_stats_id) {
    ...
    $outcome = is_array($record) ? (string) ($record['outcome'] ?? 'completed') : '';
    if (!in_array($outcome, array('completed', 'failed', 'stopped'), true)) { nmkr_set_sync_finalization_error(); return false; }
    if ($is_direct_run) {
        $owner = nmkr_get_sync_owner();
        if ($owner === false && nmkr_is_sync_terminal_status($sync_data['status'] ?? '')) {
            return nmkr_finish_ownerless_terminal_cleanup($sync_data, $outcome, $record);
        }
        if (nmkr_sync_owner_matches((string) $record['run_id'], 'running', $sync_stats_id)) {
            $transitioned = nmkr_transition_sync_owner((string) $record['run_id'], 'running', 'finalizing', $sync_stats_id);
            if (is_wp_error($transitioned)) {
                nmkr_preserve_and_schedule_sync_finalization_retry($record, true);
                return false;
            }
            if ($transitioned === false
                && !nmkr_sync_owner_matches((string) $record['run_id'], 'finalizing', $sync_stats_id)) {
                $after_transition_data = nmkr_get_sync_data();
                if (nmkr_get_sync_owner() === false && is_array($after_transition_data)
                    && (string) ($after_transition_data['run_id'] ?? '') === (string) $record['run_id']
                    && (int) ($after_transition_data['sync_stats_id'] ?? 0) === (int) $sync_stats_id
                    && nmkr_is_sync_terminal_status($after_transition_data['status'] ?? '')) {
                    return nmkr_finish_ownerless_terminal_cleanup($after_transition_data, $outcome, $record);
                }
                return false;
            }
        }
        if (nmkr_sync_owner_matches((string) $record['run_id'], 'stop_requested', $sync_stats_id)) {
            ...
            unset($stopped_final['metrics'], $stopped_final['error_message']);
    ...
    $error_message = $outcome === 'stopped'
        ? __('Synchronization stopped by user.', 'rocsi-connector-for-nmkr')
    ...
    if (is_array($current_sync_data)
        ...
        && $current_owner === false) {
        return nmkr_finish_ownerless_terminal_cleanup($current_sync_data, $outcome, $record);
    }
...
/** Direct-run outcomes are explicit; booleans are accepted only for legacy callers. */
function nmkr_normalize_sync_terminal_outcome($outcome) {
    if ($outcome === true) return 'completed';
    if ($outcome === false) return 'failed';
    return in_array($outcome, array('completed', 'failed', 'stopped'), true) ? $outcome : false;
}

/** Derive display quality from lifecycle and authoritative counters. */
function nmkr_derive_sync_terminal_result($outcome, $counters) {
    $outcome = nmkr_normalize_sync_terminal_outcome($outcome);
    if ($outcome === 'failed' || $outcome === 'stopped') return $outcome;
    if ($outcome !== 'completed' || !is_array($counters)) return false;
...
function nmkr_sync_terminal_history_statuses($outcome) {
    ...
    if ($outcome === 'failed') return array('failed', 'error');
    return $outcome === 'stopped' ? array('stopped') : array();
}
...
function nmkr_verify_sync_terminal_result($sync_data, $outcome) {
    ...
    $allowed = nmkr_sync_terminal_history_statuses($outcome);
    ...
    foreach ($counter_keys as $counter) {
        if (!isset($history[$counter], $sync_data[$counter]) || (int) $history[$counter] !== (int) $sync_data[$counter]) return false;
    }
...
function nmkr_restore_sync_finalization_retry($sync_stats_id, $resume_record) {
    if (!nmkr_is_valid_sync_finalization_record($resume_record, (string) ($resume_record['run_id'] ?? ''), $sync_stats_id)) {
        nmkr_set_sync_finalization_error();
        return false;
    }
    return nmkr_preserve_and_schedule_sync_finalization_retry($resume_record, false);
}
...
function nmkr_sync_data_complete($success = true, $error_message = '', $final = array(), $is_resume = false, $prepared = false) {
    ...
    // exact outcome is authoritative from this point onward.
    ...
    if ($run_id !== '') {
        $initial_terminal = nmkr_is_sync_terminal_status($initial_sync_data['status'] ?? '');
        if (!$initial_terminal && !nmkr_sync_owner_matches($run_id, 'finalizing', $sync_stats_id)) {
            return false;
        }
        if ($initial_terminal && nmkr_get_sync_owner() !== false
            && !nmkr_sync_owner_matches($run_id, 'finalizing', $sync_stats_id)) {
            return false;
        }
    }
    ...
    try {
        ...
        if (!nmkr_is_sync_terminal_status($sync_data['status'] ?? '') && $run_id !== ''
            && !nmkr_sync_owner_matches($run_id, 'finalizing', $sync_stats_id)) {
            return false;
        }
        ...
        if (nmkr_is_sync_terminal_status($sync_data['status'] ?? '')) {
            ...
            $compatible = in_array(($sync_data['status'] ?? ''), nmkr_sync_terminal_history_statuses($outcome), true);
            if (!$compatible) {
                nmkr_clear_sync_finalization_resume($sync_stats_id);
                return false;
            }
        ...
        $history = $wpdb->get_row($wpdb->prepare("SELECT id, status, end_time, items_processed, items_successful, items_failed, items_skipped, token_details_synced FROM $history_table WHERE id = %d", $sync_stats_id), ARRAY_A);
        ...
        $expected_statuses = nmkr_sync_terminal_history_statuses($outcome);
        if (nmkr_is_sync_terminal_status($history['status'])) {
        ...
        foreach (array('items_processed', 'items_successful', 'items_failed', 'items_skipped', 'token_details_synced') as $counter) {
            $terminal[$counter] = (int) ($history[$counter] ?? 0);
        }
