...
function nmkr_shortcode_token($atts) {
    ...
    if ( empty( $atts['token_uid'] ) ) {
        ...
        if ( ! $token_obj ) {
            return '<p>' . esc_html__( 'No tokens found for the selected project.', 'rocsi-connector-for-nmkr' ) . '</p>';
        }
    ...
    if ( ! $token ) {
        return '<p>' . esc_html__( 'Token not found.', 'rocsi-connector-for-nmkr' ) . '</p>';
    }
    ...
    $output .= '<div class="nmkr-token-meta-item"><span class="nmkr-token-status ' . esc_attr( $status_class ) . '">' . esc_html($status_label) . '</span></div>';
