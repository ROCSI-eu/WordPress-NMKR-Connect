<?php
/**
 * ROCSI Connector for NMKR - Settings Sections Module
 *
            ...
            <span id="nmkr-dashboard-sync-logging-status-text">
                ...
                echo esc_html(
                    $dashboard_sync_logging_active
                        ? __('Dashboard Sync Logging is active. Sync logs will be stored for the Dashboard Debug Logs panel.', 'rocsi-connector-for-nmkr')
                        : __('Dashboard Sync Logging is not active. To see sync logs in the Dashboard, enable Debug Logging Controls, Enable Logging to Dashboard Logs, and Enable Data Synchronization Logging.', 'rocsi-connector-for-nmkr')
                );
                ?>
            </span>
        </p>
...
function nmkr_log_to_dashboard_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e('Enable this option to store debug logs in the WordPress database. This destination is required to view sync logs in the Dashboard Debug Logs panel.', 'rocsi-connector-for-nmkr'); ?>
    </p>
...
function nmkr_sync_debug_enabled_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e('Enable this option to write data synchronization logs, including sync start, progress, completion, stop, and error entries.', 'rocsi-connector-for-nmkr'); ?>
    </p>
    <?php
}
...
function nmkr_connect_analytics_section_callback() {
    echo '<p>' . esc_html__( 'Analytics is optional and disabled by default. Local mode stores events in this WordPress database; GA4 modes send configured event data to Google. Review consent, retention, and uninstall settings for your site.', 'rocsi-connector-for-nmkr' ) . '</p>';
}
...
function nmkr_analytics_mode_field_callback() {
    ...
    <select name="nmkr_connect_options[analytics_mode]" id="nmkr_analytics_mode">
        <option value="off" <?php selected('off', $analytics_mode); ?>><?php esc_html_e( 'Off', 'rocsi-connector-for-nmkr' ); ?></option>
        <option value="custom" <?php selected('custom', $analytics_mode); ?>><?php esc_html_e( 'Custom (Plugin DB only)', 'rocsi-connector-for-nmkr' ); ?></option>
        <option value="ga4" <?php selected('ga4', $analytics_mode); ?>><?php esc_html_e( 'GA4 only (no DB)', 'rocsi-connector-for-nmkr' ); ?></option>
        <option value="both" <?php selected('both', $analytics_mode); ?>><?php esc_html_e( 'Both (GA4 + Plugin DB)', 'rocsi-connector-for-nmkr' ); ?></option>
    </select>
    <p class="description">
        <?php esc_html_e( 'off: no tracking; custom: store events in plugin DB only; ga4: send to Google Analytics 4 only; both: GA4 + plugin DB.', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_analytics_retention_days_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( 'Number of days to retain analytics data before automatic cleanup.', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_analytics_track_logged_in_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( 'Track engagement events for logged-in users (user_id will be stored).', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_analytics_require_consent_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( 'Require explicit user consent before tracking any analytics events.', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_analytics_sample_rate_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( '0 disables; 1.0 = 100% of events.', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_analytics_remove_on_uninstall_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( 'Remove all analytics data when the plugin is uninstalled.', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_analytics_debug_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( 'Enable debug logging for analytics events (requires debug logging to be enabled).', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_ga4_measurement_id_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( 'Google Analytics 4 Measurement ID (e.g., G-XXXXXXXXXX).', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
...
function nmkr_ga4_api_secret_field_callback() {
    ...
    />
    <p class="description">
        <?php esc_html_e( 'GA4 API Secret. Used server-side only when a GA4 mode and valid configuration are deliberately selected; it is never displayed or logged.', 'rocsi-connector-for-nmkr' ); ?>
    </p>
    <?php
}
